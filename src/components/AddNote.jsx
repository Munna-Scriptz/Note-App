  ...
  return (
    ...
    <div className="relative w-full sm:w-[80%] max-w-2xl mx-auto bg-primary rounded-lg px-4 py-3 shadow-md flex flex-wrap items-center justify-between">
      <div className='w-full sm:w-[500px] mb-4 sm:mb-0'>
        ...
        <div className={`${!inpValue? 'hidden' : 'visible'} flex items-start mt-4 sm:mt-10 delay-75 duration-200`}>
          <FaPen className='text-gray-500 mr-3'/>
          <textarea value={noteContent || ''} onChange={(e)=>{setNoteContent(e.target.value)}} className='bg-transparent outline-none text-white overflow-x-hidden w-full' onInput={(e)=>{e.target.style.height = "auto"; e.target.style.height = e.target.scrollHeight + "px";}} cols={55} placeholder='Your note...'></textarea>
        </div>
      </div>
      {
        updateCng == true?
        <button onClick={()=>{handleNotes() , setInpValue('') , setUpdateCng(false) , dispatch(pinNote('AllNotes/')), setIsPin(false)}} className="bg-white h-[40px] hover:bg-[#666666] hover:text-white hover:scale-[1.05] duration-[.3s] w-full sm:w-[140px] cursor-pointer text-[#202124] font-medium rounded-lg">Update Note</button>
        :
        <button onClick={()=>{handleNotes() , dispatch(pinNote('AllNotes/')), setIsPin(false)}} className="bg-white h-[40px] w-full sm:w-[140px] cursor-pointer text-[#202124] hover:bg-[#666666] hover:text-white hover:scale-[1.05] duration-[.3s] font-medium rounded-lg">Add Note</button>
      }
    </div>
